...
class AnimateTxT {
  ...
  static default: Default = {
    ...
    text: 'AnimateTxT!' /* стандартный текст для анимации */,
    point: '.' /* стандартная точка
    для обратной замены */,
    construct: {
      /* стандартные настройки для создания
      переменных с помощью new */

      replace: false /* стандартный режим анимации
      с заменой символов */,

      // стандартный режим обратной замены
      backReplace: {
        value: true /* стандартное значение обратной замены */,
        point: '.' /* стандартная точка для обратной замены */,
      },

      // стандартные настройки времени в конструкторе
      time: {
        mode: 'half',
        ms: 1500,
      },
    ...
    this.play = async (text = this.settings.text, all_time = this.settings.time.ms) => {
      ...
      if (this.settings.backReplace.value === true) {
        ...
        for (let key = 0; key < text.length + 1; key++) {
          ...
          point = point.padEnd(
            key,
            this.settings.backReplace.point,
          ); /* создание переменной для записи точек */
  ...
  static async point({
    ...
    const time =
      Math.floor(all_time / 2) / text.length; /* расчет времени в зависимости от длины  текста */
  ...
  static async line({ text = AnimateTxT.default.text, all_time = AnimateTxT.default.time } = {}) {
    ...
    for (let key: number = 0; key < text.length; key++) {
      process.stdout.write(
        '\r' + text.slice(key + 1) + text.slice(0, key) + text[key],
      ); /* печать текущей буквы */
